...
if __name__ == "__main__":
    ...
    #                emb_p_drop=0., trans_p_drop=0., head_p_drop=0.3) # 6 000 000 params
    # model = model.to("cuda")

    # model = RelViT(10, 32, channels=256, head_channels=32, num_blocks=32, patch_size=16,
    #                 emb_p_drop=0., trans_p_drop=0., head_p_drop=0) #250 000 params

    model = RelViT(NUM_CLASSES, IMAGE_SIZE, channels=256, head_channels=32, num_blocks=8, patch_size=4,
           emb_p_drop=0., trans_p_drop=0., head_p_drop=0).cuda()
    a = time.time()
    x = torch.randn(64, 3, IMAGE_SIZE, IMAGE_SIZE).cuda()
    model(x)
